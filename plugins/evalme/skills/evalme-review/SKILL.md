---
name: evalme-review
description: 复盘(M6,读取侧):assess 刷新投影 → explain 证据链(band/novelty 分层/成长曲线/stale)→ next 选题。当用户想看自己现在什么水平、为什么是这个结论、接下来该练哪道题时使用。只读事实,不摄取、不打分、不出题(缺题时输出 forge_needed 交给 evalme-forge)。
---
...
1. **assess 先行**(读前必刷):注意输出的两类警告——待打分存货(转告用户去 evalme-grade)、无题可测的 topic(forge needed)。
...
3. **next**:打印模式给出按 priority 排序的 topic + 候选题(优先 unseen/variant)。你把它包装成至多 3 条 action(`{topic, task_ref, reason}` 或 `{topic, forge_needed: true, reason}`)从 stdin `--write`。
   ...
   - `forge_needed` = 题库没合适的题,建议用户开 evalme-forge 会话补题;
