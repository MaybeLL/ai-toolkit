...
node <scripts>/evalme.mjs task show <ref|family> [--prompt-only]
node <scripts>/evalme.mjs grader add --file <grader.yaml>   # common grader
```
...
1. 读 evalme-review 给的缺口(topic + mode);**不要读该 topic 的历史 transcript 细节**——按 topic 出题,不按用户的具体弱点反向定制题面(防 teaching-to-test)。
...
5. **可主持性质检**:
   - 题面明确区分共享背景与有顺序的问题,使 practice 能一次只呈现一个问题;不得依赖工具输出或隐藏材料才能作答。
   - 若包含多个小问,它们必须服务同一个能力判断;互不依赖的知识问拆成不同 task,避免一次 trial 混成十道题而失去诊断粒度。
   - 题面声明影响答案的系统契约(例如是否存意向、计划、事件);若允许多种架构策略,grader 应接受“声明合理假设后作答”,不能一边欠约束一边要求唯一答案。
   - 候选事实中的主体、时间和语义维度必须清楚,避免把“团队内部沟通渠道”和“客户支持渠道偏好”之类不同概念伪装成冲突。
6. **展示草案并请求审批**:向用户展示题面、labels、difficulty、全部 checks 和 reference_solution，并明确询问“是否确认将此 task 入库？”。
...
4. label 不在词表时提醒用户:这可能是个新 topic,去 evalme-define 加词(顺便定 weight)。
...
task/grader 一经 grading 引用即不可变;修订 = 新版本文件(version+1,CLI 强制版本连续)。修订后可对旧 trial 重判(evalme-grade),聚合自动取最新版本判定。
