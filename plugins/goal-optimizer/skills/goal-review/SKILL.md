...
**同步(ADR-0011)**:会话开始时,若 goal home 是 git 仓库且有 remote → 先 `git pull --ff-only`,确保看的是最新事实;失败(离线/冲突)可继续,但要告知用户结论可能滞后。本 skill 只读,不主动 commit;若 assess 输出「sync debt」警告,转告用户并跑 `node <scripts>/goal.mjs sync` 清欠账。
